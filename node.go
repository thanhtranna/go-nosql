...
func NewNodeForSerialization(items []*Item, childNodes []pageNum) *Node {
	return &Node{
		items:      items,
...
func (n *Node) deserialize(buf []byte) {
	...
	for i := 0; i < itemsCount; i++ {
		if isLeaf == 0 { // False
			pgNum := binary.LittleEndian.Uint64(buf[leftPos:])
			leftPos += pageNumSize

			n.childNodes = append(n.childNodes, pageNum(pgNum))
		}
...
func (n *Node) elementSize(i int) int {
	...
	size += pageNumSize // 8 is the pageNum size
	return size
}
...
func (n *Node) split(nodeToSplit *Node, nodeToSplitIndex int) {
	...
	if nodeToSplit.isLeaf() {
		newNode = n.writeNode(n.tx.newNode(nodeToSplit.items[splitIndex+1:], []pageNum{}))
	...
	if len(n.childNodes) == nodeToSplitIndex+1 { // If middle of list, then move items forward
		n.childNodes = append(n.childNodes, newNode.pgNum)
	} else {
		n.childNodes = append(n.childNodes[:nodeToSplitIndex+1], n.childNodes[nodeToSplitIndex:]...)
		n.childNodes[nodeToSplitIndex+1] = newNode.pgNum
	}

	n.writeNodes(n, nodeToSplit)
}

// rebalancedRemove re-balances the tree after a remove operation. This can be either by rotating to the right, to the
// left or by merging. First, the sibling nodes are checked to see if they have enough items for re-balancing
...
func (n *Node) rebalancedRemove(unbalancedNode *Node, unbalancedNodeIndex int) error {
	pNode := n
...
func (n *Node) merge(bNode *Node, bNodeIndex int) error {
	...
	n.tx.db.deleteNode(bNode.pgNum)
	return nil
}
